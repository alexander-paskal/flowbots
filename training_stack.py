...
if __name__ == '__main__':
    ...
    model_unstacked_1, unused_info = load_model('FlowNetS_FlyingChairs_scheduler_long')
    model_cls = lookup['flownet-s']
    model_unstacked_2 = initialize(model_cls, in_channels=8)

    
    model = FlownetStacked(model_unstacked_1, model_unstacked_2, warping=False, frozen=[True, False])
    #    
    ...
    try:
        with open(os.path.join("weights", MODEL_NAME + ".json")) as f:
            info = json.load(f)
        model.load_state_dict(torch.load('C:\\Users\\Local User\\OneDrive - UC San Diego\\285\\project_work\\updated_flowbots\\flowbots_2\\weights\\FlowNetSS_FirstNetwork_Frozen.pth'))
        losses = info["losses"]
